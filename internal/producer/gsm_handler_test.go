...
func TestBuildNASPacketFilterFromPacketFilterInfo(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			packetFilter, err := buildNASPacketFilterFromFlowInformation(&tc.flowInfo)
			require.NoError(t, err)

			require.Equal(t, tc.packetFilter.Direction, packetFilter.Direction)
			require.ElementsMatch(t, tc.packetFilter.Components, packetFilter.Components)
		})
	}
}
