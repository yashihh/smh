...
func HandleSMPolicyUpdateNotify(smContextRef string, request models.SmPolicyNotification) *httpwrapper.Response {
	...
	if err := smContext.UpdateSessionRules(decision.SessRules); err != nil {
	...
	if err := ApplyPccRuleFromDecision(smContext, decision, true); err != nil {
...
func getTcDataFromDecision(pccRule *smf_context.PCCRule,
	...
	if tcID := pccRule.RefTrafficControlData(); tcID != "" {
		if tcModel, exist := decision.TraffContDecs[tcID]; exist && tcModel != nil {
			tcData = smf_context.NewTrafficControlData(tcModel)
...
func ApplyPccRuleFromDecision(smContext *smf_context.SMContext,
	decision *models.SmPolicyDecision, sendPFCP bool) error {
	logger.PduSessLog.Traceln("In ApplyPccRuleFromDecision")
	var err error
	if decision == nil {
		return fmt.Errorf("SmPolicyDecision not found")
	}
	...
	if len(decision.PccRules) == 0 {
		...
		for tcID, tcModel := range decision.TraffContDecs {
			...
			if tcData, exist := smContext.TrafficControlDatas[tcID]; exist {
				for _, pccRef := range tcData.RefedPCCRules() {
					...
					if err = applyTrafficRoutingData(smContext, relatedPCCRule, relatedPCCRule,
						...
						getTcDataFromDecision(relatedPCCRule, decision), sendPFCP); err != nil {
						return err
					}
				}
			} else {
	...
	for pccRuleID, pccRuleModel := range decision.PccRules {
		...
		if pccRuleModel == nil {
			...
			err = applyPCCRule(smContext, srcPccRule, nil, pccRuleID, nil, qos, sendPFCP)
		} else {
			if exist {
			...
			targetPccRule := smf_context.NewPCCRule(pccRuleModel)
			err = applyPCCRule(
				smContext, srcPccRule, targetPccRule, pccRuleID,
				getTcDataFromDecision(targetPccRule, decision), qos, sendPFCP)
		}
	}

	logger.PduSessLog.Traceln("End of ApplyPccRuleFromDecision")
	return err
}

func applyPCCRule(smContext *smf_context.SMContext, srcPccRule, targetPccRule *smf_context.PCCRule,
	pccRuleID string, tcData *smf_context.TrafficControlData, qos *models.QosData, sendPFCP bool) error {
	if targetPccRule == nil { // Remove PCC Rule
		if srcPccRule != nil {
			if err := applyTrafficRoutingData(smContext, srcPccRule, nil,
				srcPccRule.RefTrafficControlData(), nil, sendPFCP); err != nil {
				return err
			}
	...
	if err := applyTrafficRoutingData(smContext, srcPccRule, targetPccRule,
		targetPccRule.RefTrafficControlData(), tcData, sendPFCP); err != nil {
		return err
	}
...
func applyTrafficRoutingData(smContext *smf_context.SMContext, srcPccRule, targetPccRule *smf_context.PCCRule,
	applyTcID string, targetTcData *smf_context.TrafficControlData, sendPFCP bool) error {
	trChanged := false
	...
	if applyTcID != "" {
		srcTcData, exist := smContext.TrafficControlDatas[applyTcID]
		if exist {
		...
		if targetTcData != nil {
			...
			smContext.TrafficControlDatas[applyTcID] = targetTcData
		} else { // Remove traffic control data
			delete(smContext.TrafficControlDatas, applyTcID)
		}
	} else {
